...

{
    ...
    SpringArmComponent->SetupAttachment(GetRootComponent()); //вращение камеры относительно песронажа
    SpringArmComponent->bUsePawnControlRotation = true;
    SpringArmComponent->SocketOffset = FVector(0.0f, 100.0f, 80.0f);
   
    CameraComponent = CreateAbstractDefaultSubobject<UCameraComponent>("CameraComponent");
    CameraComponent->SetupAttachment(SpringArmComponent); // camera
...
float ASTUBaseCharacter::GetMovementDirection() const
{
    ...
    const auto AngeBetween = FMath::Acos(FVector::DotProduct(GetActorForwardVector(), VelocityNormal));
